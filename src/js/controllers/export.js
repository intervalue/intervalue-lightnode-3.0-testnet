	...
	function($rootScope, $scope, $timeout, $log, $filter, backupService, storageService, fileSystemService, isCordova, isMobile, gettextCatalog, notification) {
		...
		function saveFile(file, cb) {
			...
			if (!isCordova) {
				...
				fileSystemService.cordovaWriteFile((isMobile.iOS() ? window.cordova.file.cacheDirectory : window.cordova.file.externalRootDirectory), 'Intervalue', backupFilename, file, function(err) {
					if (isMobile.iOS()) {
						navigator.notification.alert(gettextCatalog.getString('Now you have to send this file somewhere to restore from it later ("Save to Files", send to yourself using chat apps, etc.)'), function(){
							window.plugins.socialsharing.shareWithOptions({files: [window.cordova.file.cacheDirectory + 'Intervalue/'+ backupFilename]}, function(){}, function(){});
						}, 'Backup done');
					}
					cb(err);
				});
